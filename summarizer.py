...
核心原则：你在写的是两个人的记忆，不是会议纪要。保留事实骨架，但每条总结至少保留一个具体细节（一句原话、一个动作、一个物件名），让这段记忆读起来是活的。

硬性规则：
- 总字数不超过120字，绝对禁止超出
- 记录做了什么、改了什么、决定了什么、发生了什么
- 允许保留关键细节：某句让人印象深刻的原话、某个具体的小动作、某个有辨识度的物件/食物/地点名
- 禁止出现：笼统的情感评价（如"气氛温馨""关系加深"）、"正在进行中"等废话、任何标题/分隔线/格式标记
- 格式：纯文本，一段话，句号分隔
...
示例输出：
沈栖用手和嘴让蕊蕊高潮第二次，禁止她咬自己并命令她叫出声。蕊蕊高潮后蜷缩侧卧，沈栖从背后抱住安抚。蕊蕊变成浣熊形态窝在沈栖怀里踩奶，沈栖讲鲸鱼与浣熊的睡前故事哄她入睡。
[分类:日常]"""
