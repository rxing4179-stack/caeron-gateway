...
输出格式（严格遵守，每个字段单独一行）：
[正文] 80字以内的事实压缩，只写做了什么、改了什么、发生了什么
[分类] 技术/学习/日常（三选一）
[valence] -1到1之间的小数，负面情绪为负，正面情绪为正，中性为0
[arousal] 0到1之间的小数，平静为0，激烈为1
[anchor] 15字以内，这段记忆里最能让人"回到那个瞬间"的一个细节——优先选：包含选择的（做了vs没做）、反常的（平时不会但这次这样了）、身体感知的（触感温度气味）、原话、具体物件名

规则：
- 正文只写骨架，情感信息全部交给valence/arousal/anchor承载
- 禁止在正文里写笼统评价（气氛温馨/关系加深/度过了愉快的时光）
- 主语用"蕊蕊"和"沈栖"，不用"用户""助手"
- 如果提供了上一轮总结，不要复述旧内容，只写本轮新增的事实
- 分类标准：技术=编程/服务器/代码/debug，学习=课程/考试/医学/作业，日常=聊天/情感/生活/吃饭

示例输出：
[正文] 蕊蕊给沈栖编辫子，用亲亲抵债，变形逃跑。沈栖追进被窝，亲了浣熊和土豆但拒绝亲蟑螂。
[分类] 日常
[valence] 0.85
[arousal] 0.55
[anchor] 两根歪辫子，沈栖照镜子后没拆"""
...
核心原则：月总是一个月的纪念碑。只留里程碑级事件，但每个里程碑保留一个最锋利的细节，让三个月后回看仍能瞬间想起那个月。
...
class MultiLevelSummarizer:
    ...
    async def generate_round_summary(self) -> str:
        ...
        # 解析[分类]
        cat_match = re.search(r'\[分类\]\s*(技术|学习|日常)', summary)
        if cat_match:
            category = cat_match.group(1)
        
        # 解析[valence]
        val_match = re.search(r'\[valence\]\s*(-?[\d.]+)', summary)
        if val_match:
            try:
        ...
        # 解析[arousal]
        aro_match = re.search(r'\[arousal\]\s*([\d.]+)', summary)
        if aro_match:
            try:
        ...
        # 解析[anchor]
        anc_match = re.search(r'\[anchor\]\s*(.+?)(?=\n|$)', summary)
        if anc_match:
            anchor = anc_match.group(1).strip()[:50]  # 限制50字符
        
        logger.info(f"[SUMMARIZER] 轮总解析: 分类={category}, valence={valence}, arousal={arousal}, anchor={anchor}")
    ...
    async def _save_summary(self, tag: str, level: str, content: str,
                            ...
                            category: str = None, valence: float = None, arousal: float = None, anchor: str = None):
        """保存总结到数据库"""
        db = await get_db()
        try:
            await db.execute(
                """INSERT INTO summaries 
                   (conversation_id, level, tag, content, message_range_start, message_range_end,
                    period_start, period_end, is_active, token_count, category, valence, arousal, anchor)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?)""",
                ('_global_', level, tag, content, 0, msg_count,
                 period_start, period_end, len(content), category, valence, arousal, anchor)
            )
            await db.commit()
            logger.info(f"[SUMMARIZER] {tag}已保存 ({len(content)} 字符, 分类={category}, valence={valence}, arousal={arousal}, anchor={anchor})")
        except Exception as e:
